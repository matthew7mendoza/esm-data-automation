...
class TestFrontendAPIClient:
    ...
    @patch("frontend.api.requests.get")
    def test_fetch_server_templates_fallback_on_timeout(self, mock_get: MagicMock) -> None:
    ...
    def test_get_task_profile_success(self, mock_get: MagicMock) -> None:
        ...
        assert result == {"task_id": "1234", "status": "COMPLETED"}

    @patch("frontend.api.requests.get")
    def test_get_task_profile_suppresses_error_and_returns_none(self, mock_get: MagicMock) -> None:
    ...
    @patch("frontend.api.requests.get")
    def test_fetch_all_historical_tasks_returns_empty_list_on_fail(self, mock_get: MagicMock) -> None:
        mock_response: MagicMock = MagicMock()
        mock_response.status_code = 500
        mock_get.return_value = mock_response
