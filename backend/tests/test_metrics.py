...
class TestMetricsEngine:
    ...
    def test_calculate_percentage_agreement_valid(
        self, judgments: list[str], expected: float
    ) -> None:
    ...
    def test_calculate_gwets_ac1_boundaries(
        self, judgments: list[str], expected_range: tuple[float, float]
    ) -> None:
        result = calculate_gwets_ac1(judgments)
        assert expected_range[0] <= result <= expected_range[1]

    @pytest.mark.parametrize(
        "justifications, strategy, expected",
        [
            (
                ["The value is 42.", "Found 42 on line 1.", "It was 100."],
                "Numeric",
                66.67,
            ),
            (
                ["It says 'hello' here.", "We read 'hello' now.", "Nothing."],
                "Quote",
                66.67,
            ),
