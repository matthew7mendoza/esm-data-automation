...
class DocumentGenerator:
    ...
    __slots__ = ("instructions", "provider", "templates")
    ...
    def __str__(self) -> str:
        ...
        return f"Document Generator Module [Bound to Engine: {engine_name}]"
    ...
    def _load_configuration_blueprints(self) -> tuple[str, dict[str, TemplateConfig]]:
        ...
        if not config_path.exists():
            raise AgentConfigurationError(
                f"The templates.yaml file is missing at: {config_path.resolve()}"
            )
        ...
        try:
            ...
            raise AgentConfigurationError(
                "Unable to read / parse templates.yaml file. "
                "Check that format is correct."
            ) from document_read_fault

    def execute_extraction(
        self, target_questions: list[str], content_payload: str
    ) -> ExtractionReport:
        """
        ...
        formatted_questions = "\n".join(
            f"- {question}" for question in target_questions
        )
        user_prompt = (
        ...
        try:
            ...
            TypeError,
            RuntimeError,
        ) as api_communication_fault:
            logger.error(
                "Failed to communicate with AI API backend model engines!",
                exc_info=True,
            )
            raise AgentExecutionError(
                "Could not get a response from the AI model API"
            ) from api_communication_fault

        flat_answers = {
            item.question: item.answer for item in validated_data.extracted_answers
        }
        return {
    ...
    def generate_draft_from_directory(
        ...
        """
        Finds all supported files in a folder, extracts their text,
        combines them together, and sends them to the AI.
        """

        if not (active_template := self.templates.get(target_document_type.upper())):
            raise ValueError(
                f"The document type '{target_document_type}' "
                "was not found in templates.yaml"
            )
        ...
        if not input_path.exists():
            ...
            logger.info(
                "Folder was missing, so a new empty folder was created: "
        ...
        if not valid_workspace_files:
            raise AgentExecutionError(
                "Finished checking the folder, no text could successfully "
        ...
        if not aggregated_text_blocks:
            raise AgentExecutionError(
                "Finished checking the folder, no text could be "
